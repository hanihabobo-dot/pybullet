...
class BoxelRegistry:
    ...
    def generate_pddl_facts(self) -> List[str]:
        """
        Generate PDDL facts for PDDLStream integration.
        
        Returns a list of PDDL fact strings.
        """
        facts = []
        
        for boxel in self.boxels.values():
            # Basic boxel facts
            facts.append(f"(semantic_zone {boxel.id})")
            facts.append(f"(boxel_type {boxel.id} {boxel.boxel_type.value})")
            
            # Neighbor relationships
            for direction, neighbor_ids in boxel.neighbor_ids.items():
                for neighbor_id in neighbor_ids:
                    facts.append(f"(neighbor {boxel.id} {neighbor_id})")
            
            # Occlusion relationships
            if boxel.boxel_type == BoxelType.OBJECT and boxel.is_occluder:
                facts.append(f"(is_occluder {boxel.id})")
                for shadow_id in boxel.shadow_boxel_ids:
                    facts.append(f"(occludes {boxel.id} {shadow_id})")
            
            if boxel.boxel_type == BoxelType.SHADOW and boxel.created_by_boxel_id:
                facts.append(f"(occluded_by {boxel.id} {boxel.created_by_boxel_id})")
            
            # Belief state
            for obj in boxel.possibly_contains:
                facts.append(f"(possibly_contains {boxel.id} {obj})")
            
            if boxel.confirmed_contains:
                facts.append(f"(obj_in_boxel {boxel.confirmed_contains} {boxel.id})")
            
            if boxel.confirmed_empty:
                facts.append(f"(confirmed_empty {boxel.id})")
            
            # Reachability
            if boxel.is_reachable:
                facts.append(f"(reachable {boxel.id})")
            
            if boxel.is_observable:
                facts.append(f"(observable {boxel.id})")
    ...
    def save_pddl_facts(self, filepath: str) -> None:
        ...
        with open(filepath, 'w') as f:
            f.write(";; Boxel PDDL Facts\n")
            f.write(";; Auto-generated by boxel_data.py\n\n")
            for fact in facts:
                f.write(f"{fact}\n")
